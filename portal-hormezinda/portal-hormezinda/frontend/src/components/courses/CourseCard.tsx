...
export function CourseCard({ canManage, course, onDelete, onEdit, onOpen }: CourseCardProps) {
  return (
    <article className="group overflow-hidden rounded-3xl border border-slate-950/5 bg-white shadow-card ring-1 ring-white/80 transition duration-200 hover:-translate-y-1 hover:shadow-hover">
      <button className="block w-full text-left" onClick={() => onOpen(course)} type="button">
        <div className="relative aspect-[16/10] bg-slate-100">
          ...
          <span className="absolute left-3 top-3 rounded-full bg-white px-3 py-1 text-xs font-bold text-brand-navy shadow-card">
            {course.categoria}
          </span>
        ...
        <div className="space-y-4 p-5">
          <div>
            <h2 className="line-clamp-2 text-xl font-semibold tracking-normal text-brand-navy">
              {course.titulo}
            </h2>
      ...
      {canManage ? (
        <div className="flex gap-2 border-t border-slate-100 p-4">
          <button
            className="inline-flex flex-1 items-center justify-center gap-2 rounded-2xl bg-slate-50 px-3 py-2 text-sm font-semibold text-brand-navy transition hover:bg-blue-50 hover:text-brand-blue"
          ...
          <button
            className="inline-flex flex-1 items-center justify-center gap-2 rounded-2xl bg-slate-50 px-3 py-2 text-sm font-semibold text-red-600 transition hover:bg-red-50"
