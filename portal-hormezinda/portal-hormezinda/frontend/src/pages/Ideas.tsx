...
export function Ideas() {
  ...
  const [adminDrafts, setAdminDrafts] = useState<
    Record<string, { respostaOficial: string; status: IdeaStatus; destaque: boolean }>
  >({});

  const totalSupports = useMemo(
    () => ideas.reduce((total, idea) => total + idea.quantidadeApoios, 0),
    [ideas],
  );
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-7xl space-y-5">
        <header className="overflow-hidden rounded-[2rem] border border-slate-950/5 bg-white shadow-card ring-1 ring-white/80">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  <span className="rounded-full bg-blue-50 px-3 py-1.5 text-xs font-bold text-brand-blue">
                    {totalSupports} apoios
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="grid gap-3 lg:grid-cols-[1fr_12rem_13rem_12rem]">
            <label className="block" htmlFor="idea-search">
              ...
              <div className="mt-2 flex items-center rounded-2xl border border-slate-200 px-4 focus-within:ring-4 focus-within:ring-blue-100">
                ...
                <input
                  className="h-11 w-full bg-transparent px-3 text-sm font-medium outline-none"
                  id="idea-search"
            ...
            <Select
              label="Ordenar"
              name="sort"
              onChange={(event) => setSort(event.target.value as 'recentes' | 'apoiadas')}
              value={sort}
            >
        ...
        {!isLoading && !ideas.length ? (
          <EmptyState
            description="Compartilhe a primeira proposta da comunidade."
            icon={Lightbulb}
          ...
          <section className="grid gap-4 xl:grid-cols-2">
            {ideas.map((idea) => (
              <IdeaCard
                ...
                isAdmin={isAdmin}
                key={idea.id}
                onAdminDraftChange={(draft) =>
                  setAdminDrafts((current) => ({
                    ...current,
                    [idea.id]: { ...current[idea.id], ...draft },
                  }))
                }
      ...
      >
        <form className="space-y-4" onSubmit={(event) => void handleSubmit(event)}>
          ...
          <Select
            ...
            name="categoria"
            onChange={(event) =>
              setForm((current) => ({ ...current, categoria: event.target.value as IdeaCategory }))
            }
          ...
          <label className="block rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-5 text-center transition hover:border-brand-blue hover:bg-blue-50/40">
            ...
            <span className="mt-2 block text-sm font-semibold text-brand-navy">
              {form.imagem?.name ?? 'Selecionar imagem'}
            </span>
            ...
            <input
              ...
              className="sr-only"
              onChange={(event) =>
                setForm((current) => ({ ...current, imagem: event.target.files?.[0] }))
              }
              type="file"
            />
          </label>
  ...
  return (
    <section className="grid gap-4 xl:grid-cols-2">
      {Array.from({ length: 4 }).map((_, index) => (
        <div
          className="rounded-3xl border border-slate-950/5 bg-portal-surface p-5 shadow-card ring-1 ring-white/80"
          key={index}
        >
...
type IdeaCardProps = {
  ...
  isAdmin: boolean;
  onAdminDraftChange: (
    draft: Partial<{ destaque: boolean; respostaOficial: string; status: IdeaStatus }>,
  ) => void;
  onAdminUpdate: () => void;
  onDelete: () => void;
  onEdit: () => void;
...
function IdeaCard({
  ...
  return (
    <article className="overflow-hidden rounded-3xl border border-slate-950/5 bg-white shadow-card ring-1 ring-white/80 transition duration-200 hover:-translate-y-0.5 hover:shadow-soft">
      {idea.autor.bannerPerfil ? (
        <div
          className="h-24 bg-slate-100 bg-cover bg-center"
          style={{ backgroundImage: `url(${getAssetUrl(idea.autor.bannerPerfil)})` }}
        />
      ) : null}
      <div className="space-y-5 p-5 sm:p-6">
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs font-semibold text-slate-500">
                {getDisplayRoleLabel(idea.autor)} - {formatDate(idea.criadaEm)}
              </p>
            </div>
          </div>
          <div className="flex shrink-0 flex-wrap justify-end gap-2">
            {idea.destaque ? (
              <span className="inline-flex items-center gap-1 rounded-full bg-yellow-50 px-3 py-1 text-xs font-bold text-yellow-700 ring-1 ring-yellow-100">
        ...
        <div>
          <span className="inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">
            {ideaCategoryLabels[idea.categoria]}
          </span>
          <h2 className="mt-3 text-2xl font-semibold tracking-normal text-brand-navy">
            {idea.titulo}
          </h2>
        ...
        {idea.imagem ? (
          <img
            alt={idea.imagem.alt || idea.titulo}
            className="max-h-[34rem] w-full rounded-3xl object-cover ring-1 ring-slate-100"
            decoding="async"
            loading="lazy"
            src={getAssetUrl(idea.imagem.url)}
          />
        ) : null}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0">
                <p className="text-sm font-bold text-brand-blue">
                  {getDisplayRoleLabel(idea.respostaOficial.autor)}
                </p>
                <p className="truncate text-sm font-semibold text-brand-navy">
                  {idea.respostaOficial.autor.nomeCompleto}
                </p>
              </div>
            </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button
            onClick={onSupport}
            type="button"
            variant={idea.apoiadaPeloUsuario ? 'primary' : 'secondary'}
          >
          ...
          {canManage ? (
            <>
              <Button className="px-3" onClick={onEdit} type="button" variant="secondary">
        ...
        <PostFooter
          isReacting={false}
          myReaction={idea.minhaReacao}
          onReact={onReact}
          reactions={idea.reacoes}
        />
        ...
        {canOfficiallyRespond && adminDraft ? (
          <div className="grid gap-3 rounded-3xl border border-slate-100 bg-slate-50 p-4">
            {isAdmin ? (
              <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
                <Select
                  ...
                  name={`status-${idea.id}`}
                  onChange={(event) =>
                    onAdminDraftChange({ status: event.target.value as IdeaStatus })
                  }
                ...
                <label className="mt-7 flex h-11 items-center gap-2 rounded-2xl bg-white px-4 text-sm font-semibold text-brand-navy ring-1 ring-slate-100">
                  <input
                    checked={adminDraft.destaque}
                    className="h-4 w-4 accent-brand-blue"
                    onChange={(event) => onAdminDraftChange({ destaque: event.target.checked })}
                    type="checkbox"
                  />
                  Destaque
                </label>
            ...
            <Textarea
              ...
              name={`response-${idea.id}`}
              onChange={(event) => onAdminDraftChange({ respostaOficial: event.target.value })}
              rows={3}
              value={adminDraft.respostaOficial}
            />
