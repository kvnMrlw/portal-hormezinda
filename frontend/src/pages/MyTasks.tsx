            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="mt-2 text-sm font-medium text-slate-500">
                  Atividades ordenadas por data de entrega.
                </p>
              </div>
            </div>
            <Select
              ...
              <Card className="p-5 shadow-sm" key={task.id}>
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <div className="flex flex-wrap gap-2">
                      ...
                      <Badge variant={task.entrega ? 'success' : isLate ? 'error' : 'neutral'}>
                        {task.entrega ? 'Entregue' : isLate ? 'Atrasada' : 'Pendente'}
                      </Badge>
                    </div>
                    <h2 className="mt-3 text-xl font-semibold text-brand-navy">{task.titulo}</h2>
                    <p className="mt-1 text-sm font-semibold text-slate-500">
                      {task.disciplina.nome} · {formatClassName(task.turma)}
                    </p>
                  </div>
                  {task.entrega ? (
                ...
                <div className="mt-4 flex flex-wrap items-center gap-3 text-sm font-semibold text-slate-500">
                  ...
                  {task.arquivo ? (
                    <a
                      ...
                      href={getAssetUrl(task.arquivo.url)}
                      rel="noreferrer"
                      target="_blank"
                    >
                      Arquivo da tarefa
                  ...
                  {task.entrega ? (
                    <a
                      className="text-brand-blue"
                      href={getAssetUrl(task.entrega.arquivo.url)}
                      rel="noreferrer"
                      target="_blank"
                    >
                      Sua entrega
                ...
                <label className="mt-5 inline-flex">
                  <input
                    ...
                    onChange={(event) => void handleSubmit(task, event)}
                    type="file"
                  />
