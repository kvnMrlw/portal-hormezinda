...
export function Menu() {
  ...
  const mealsByDay = useMemo(
    () =>
      weekdays.reduce<Record<Weekday, Meal[]>>(
        (groups, weekday) => {
  ...
  return (
    <AppShell>
            ...
            {weekdays.map((weekday) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  {isAdmin ? (
                    <button
                      className="rounded-full bg-slate-50 p-2 text-brand-blue transition hover:bg-blue-50"
                ...
                <div className="mt-4 space-y-3">
                  {mealsByDay[weekday].length ? (
                    mealsByDay[weekday].map((meal) => (
                      <article
                      ...
                      >
                        <div className="aspect-[4/3] bg-white">
                          {meal.imagem ? (
                            <img
                              alt={meal.imagem.alt || meal.nome}
                              className="h-full w-full object-cover"
                              src={getAssetUrl(meal.imagem.url)}
                            />
                          ) : (
                        ...
                        <div className="p-3">
                          ...
                          {isAdmin ? (
                            <div className="mt-3 flex gap-2 border-t border-slate-200 pt-3">
                              <button
                                className="flex-1 rounded-xl bg-white px-3 py-2 text-sm font-semibold text-brand-navy transition hover:text-brand-blue"
                              ...
                              <button
                                className="flex-1 rounded-xl bg-white px-3 py-2 text-sm font-semibold text-red-600 transition hover:bg-red-50"
      ...
      <Modal
        ...
        title={editingMeal ? 'Editar cardapio' : 'Novo cardapio'}
      >
        <form className="space-y-4" onSubmit={(event) => void handleSubmit(event)}>
          <div className="grid gap-4 sm:grid-cols-2">
            <Input
              ...
              name="nome"
              onChange={(event) =>
                setPayload((current) => ({ ...current, nome: event.target.value }))
              }
              required
              value={payload.nome}
            />
            <Select
              ...
              name="diaSemana"
              onChange={(event) =>
                setPayload((current) => ({ ...current, diaSemana: event.target.value as Weekday }))
              }
              value={payload.diaSemana}
            >
              {weekdays.map((weekday) => (
            ...
            <Select
              label="Tipo da refeicao"
              name="categoria"
              onChange={(event) =>
                setPayload((current) => ({
                  ...current,
                  categoria: event.target.value as MealCategory,
                }))
              }
              value={payload.categoria}
            >
              {[MealCategory.SNACK, MealCategory.LUNCH, MealCategory.DINNER].map((category) => (
            ...
            <label className="block">
              ...
              <input
                ...
                onChange={(event) =>
                  setPayload((current) => ({ ...current, imagem: event.target.files?.[0] }))
                }
