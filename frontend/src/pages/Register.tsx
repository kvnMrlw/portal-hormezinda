...
export function Register() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setErrors({ form: 'Não foi possível cadastrar. Tente outro usuário.' });
    } finally {
        ...
        <section className="login-v15__form-side register-v27__form-side">
          ...
          <div className="login-v15__form-wrap register-v27__form-wrap">
            <div className="register-v27__topline">
              <div className="login-v15__kicker register-v27__kicker">
                <Sparkles />
                Criar acesso
              </div>

              <Link className="register-v27__login-link" to="/login">
                Já tenho conta
              </Link>
            </div>
            ...
            <div className="register-v27__intro">
              <p>Preencha seus dados para entrar na comunidade escolar.</p>

              <div className="register-v27__progress" aria-label={`Etapa ${step} de 4`}>
                {[1, 2, 3, 4].map((item) => (
                  <span
                    className={[
                      'register-v27__progress-step',
                      item < step ? 'is-done' : '',
                      item === step ? 'is-active' : '',
                    ]
                      .filter(Boolean)
            ...
            <form noValidate onSubmit={handleSubmit}>
              <div className="register-v27__step-header">
                ...
                <strong>
                  {step === 1 ? 'Seus dados' : null}
                  {step === 2 ? 'Seu turno' : null}
                  {step === 3 ? 'Sua turma' : null}
                  {step === 4 ? 'Seu acesso' : null}
                </strong>
              </div>
              ...
              {step === 1 ? (
                <div className="register-v27__fields">
                  <label className="login-v15__field" htmlFor="nomeCompleto">
                    ...
                    {errors.nomeCompleto ? (
                      <span className="login-v15__error">
                        <TriangleAlert />
                        {errors.nomeCompleto}
                      </span>
                    ) : null}
                  ...
                  <label className="login-v15__field" htmlFor="dataNascimento">
                    ...
                    <div className="login-v15__input register-v27__date-input">
                      <CalendarDays />
                      <input
                        disabled={isSubmitting}
                        id="dataNascimento"
              ...
              {step === 2 ? (
                <div className="register-v27__choice-grid register-v27__choice-grid--turno">
                  {[Turno.MATUTINO, Turno.VESPERTINO].map((turno) => (
                    <button
                      className={`register-v27__choice ${
                        formData.turno === turno ? 'is-selected' : ''
                      }`}
                      key={turno}
                      onClick={() => {
                      ...
                      type="button"
                    >
                      <span className="register-v27__choice-icon">
                        <GraduationCap />
                      </span>
                      <span>
                        <strong>{turno}</strong>
                      ...
                      <span className="register-v27__choice-check">
                        <Check />
                      </span>
                    </button>
                  ))}
                  ...
                  {errors.turno ? (
                    <span className="login-v15__error register-v27__wide-error">
                      <TriangleAlert />
                      {errors.turno}
                    </span>
                  ) : null}
              ...
              {step === 3 ? (
                <div className="register-v27__class-wrap">
                  <p className="register-v27__helper">
                    Escolha sua turma do período {formData.turno.toLowerCase()}.
                  </p>

                  <div className="register-v27__class-grid">
                    {availableTurmas.map((turma) => (
                      <button
                        className={`register-v27__class-button ${
                          formData.turma === turma ? 'is-selected' : ''
                        }`}
                        key={turma}
              ...
              <div className="register-v27__actions">
                {step > 1 ? (
                  ...
                  <Link className="register-v27__back" to="/">
                    <ArrowLeft />
                    <span>Voltar</span>
                  </Link>
                )}
                ...
                {step < 4 ? (
                  <button
                    className="login-v15__submit register-v27__next"
                    disabled={isSubmitting}
                    onClick={handleNext}
                    type="button"
                  >
            ...
            <div className="login-v15__safe">
              <ShieldCheck />
              Cadastro protegido pelo Portal Hormezinda
            </div>
          </div>
