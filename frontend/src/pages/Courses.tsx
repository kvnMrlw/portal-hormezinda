...
function contentFromCourse(course: Course): ContentDraft[] {
  return course.conteudos
    .filter(
      (content) =>
        content.tipo === CourseContentType.TEXT || content.tipo === CourseContentType.LINK,
    )
    .map((content) => ({
      link: content.link ?? '',
      ordem: content.ordem,
...
export function Courses() {
  ...
  const categories = useMemo(
    () =>
      Array.from(new Set(courses.map((course) => course.categoria)))
        .filter(Boolean)
        .sort(),
    [courses],
  );
  ...
  function openCreate(): void {
    ...
    setPayload({
      ...emptyPayload,
      professorId: isAdmin ? (teachers[0]?.id ?? user?.id ?? '') : (user?.id ?? ''),
    });
    setContents([]);
    setIsModalOpen(true);
  }
    ...
    <AppShell>
      <div className="mx-auto max-w-7xl space-y-5">
        ...
        {!isLoading && !courses.length ? (
          <EmptyState
            description="Nenhum material interno encontrado."
            icon={BookOpenCheck}
        ...
        {!isLoading && courses.length ? (
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {courses.map((course) => (
              <CourseCard
                canManage={isAdmin || course.professor.id === user?.id}
                course={course}
                key={course.id}
        ...
        <form className="space-y-5" onSubmit={(event) => void handleSubmit(event)}>
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Input
              ...
              name="categoria"
              onChange={(event) =>
                setPayload((current) => ({ ...current, categoria: event.target.value }))
              }
              required
            ...
            {isAdmin ? (
              <Select
                ...
                name="professorId"
                onChange={(event) =>
                  setPayload((current) => ({ ...current, professorId: event.target.value }))
                }
                required
                value={payload.professorId}
              >
            ...
            <Input
              ...
              name="link"
              onChange={(event) =>
                setPayload((current) => ({ ...current, link: event.target.value }))
              }
              type="url"
              value={payload.link ?? ''}
            />
            <Input
              ...
              name="capa"
              onChange={(event) =>
                setPayload((current) => ({ ...current, capa: event.target.files?.[0] }))
              }
              type="file"
            />
            <div className="md:col-span-2">
              <Textarea
                ...
                name="descricao"
                onChange={(event) =>
                  setPayload((current) => ({ ...current, descricao: event.target.value }))
                }
                required
                rows={4}
          ...
          <section className="rounded-3xl border border-slate-100 bg-slate-50 p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <Button
                className="px-3 py-2"
                onClick={() =>
                  setContents((current) => [...current, { ...emptyContent, ordem: current.length }])
                }
                type="button"
                variant="secondary"
              >
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-3" key={index}>
                  <div className="grid gap-3 md:grid-cols-[1fr_10rem_auto]">
                    ...
                    <Select
                      ...
                      onChange={(event) =>
                        setContents((current) =>
                          current.map((item, itemIndex) =>
                            itemIndex === index
                              ? { ...item, tipo: event.target.value as CourseContentType }
                              : item,
                          ),
                        )
                      }
                    ...
                    <button
                      ...
                      className="mt-7 inline-flex h-11 items-center justify-center rounded-2xl bg-red-50 px-3 text-red-600"
                      onClick={() =>
                        setContents((current) =>
                  ...
                  {content.tipo === CourseContentType.LINK ? (
                    ...
                    <div className="mt-3">
                      <Textarea
                        label="Texto"
                        name={`content-text-${index}`}
                        onChange={(event) =>
                          setContents((current) =>
                            current.map((item, itemIndex) =>
                              itemIndex === index ? { ...item, texto: event.target.value } : item,
                            ),
                          )
                        }
                        rows={3}
    ...
    >
      <div className="-mx-6 -mt-5">
        <div className="relative aspect-[16/7] bg-slate-100">
          {course.capa ? (
            <img
              alt={course.capa.alt || course.titulo}
              className="h-full w-full object-cover"
              src={getAssetUrl(course.capa.url)}
            />
          ) : null}
        </div>
      </div>
      <div className="space-y-5 pt-5">
        <div>
          ...
          <p className="mt-2 text-sm font-semibold text-slate-500">
            Responsavel: {course.professor.nomeCompleto}
          </p>
          <p className="mt-4 leading-7 text-slate-600">{course.descricao}</p>
          {course.link ? (
            <a
              className="mt-4 inline-flex items-center gap-2 text-sm font-bold text-brand-blue"
              href={course.link}
              rel="noreferrer"
              target="_blank"
            >
          ...
          {course.conteudos.map((content) => {
            ...
            return (
              <article
                ...
                key={content.id ?? `${content.titulo}-${content.ordem}`}
              >
                <div className="flex items-center gap-3">
                  <span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-white text-brand-blue ring-1 ring-slate-100">
                    <Icon className="h-5 w-5" />
                  </span>
                  <div>
                    <h3 className="font-semibold text-brand-navy">{content.titulo}</h3>
                    <p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-400">
                      {courseContentTypeLabels[content.tipo]}
                    </p>
                  </div>
                </div>
                {content.texto ? (
                ...
                {content.link ? (
                  <a
                    className="mt-3 inline-flex text-sm font-bold text-brand-blue"
                    href={content.link}
                    rel="noreferrer"
                    target="_blank"
                  >
                ...
                {content.arquivo ? (
                  <a
                    className="mt-3 inline-flex text-sm font-bold text-brand-blue"
                    href={getAssetUrl(content.arquivo.url)}
                    rel="noreferrer"
                    target="_blank"
                  >
                    Abrir arquivo
