...
export function ScheduleLessonCard({
  ...
  const textColor = isInterval
    ? '#475569'
    : getReadableTextColor(schedule.disciplina?.cor ?? '#2563eb');
  ...
  return (
    <article
      className={cn(
        ...
        happening
          ? 'ring-2 ring-brand-blue shadow-[0_0_0_4px_rgba(37,99,235,0.12)]'
          : 'ring-black/5',
      ...
      style={
        isInterval
          ? undefined
          : { backgroundColor: schedule.disciplina?.cor ?? '#2563eb', color: textColor }
      }
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div
            className="flex items-center gap-1.5 text-[0.68rem] font-bold uppercase tracking-[0.12em]"
            style={{ color: isInterval ? '#64748b' : textColor }}
          >
            <Icon className="h-4 w-4 shrink-0" />
            <span>{formatTimeRange(schedule)}</span>
          </div>
          {happening ? (
            <span className="mt-2 inline-flex rounded-full bg-white px-2.5 py-1 text-xs font-bold text-brand-blue shadow-sm">
              Aula em andamento
            </span>
          ) : null}
        </div>
        ...
        {canManage ? (
          <div className="flex shrink-0 gap-1 opacity-100 transition sm:opacity-0 sm:group-hover:opacity-100">
            <button
              ...
              className="rounded-full bg-white p-1.5 text-slate-600 shadow-card transition hover:-translate-y-0.5"
            ...
            <button
              ...
              className="rounded-full bg-white p-1.5 text-red-600 shadow-card transition hover:-translate-y-0.5"
      ...
      <h3
        className={cn(
          'mt-2 truncate font-semibold leading-tight',
      ...
      <div
        className="mt-2 flex flex-wrap gap-1.5 text-[0.68rem] font-bold"
        style={{ color: textColor }}
      >
        {schedule.sala ? (
          <span className="rounded-full bg-white/20 px-2 py-0.5">{schedule.sala.nome}</span>
        ) : null}
        {schedule.turma ? (
          <span className="rounded-full bg-white/20 px-2 py-0.5">
            {formatClassName(schedule.turma)}
          </span>
        ) : null}
      </div>
    </article>
