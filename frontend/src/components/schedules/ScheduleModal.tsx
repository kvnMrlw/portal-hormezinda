...
export function ScheduleModal({
  ...
  const [form, setForm] = useState<ScheduleFormState>(() =>
    getInitialState(schedule, initialClassId, initialWeekday, initialStart, initialEnd),
  );
  const isLesson = form.tipo === ScheduleEntryKind.LESSON;
  const isEditing = mode === 'edit';
  const title =
    mode === 'edit' ? 'Editar horario' : mode === 'duplicate' ? 'Duplicar horario' : 'Novo horario';
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {!isEditing ? (
            <Select
              ...
              onChange={(event) =>
                setForm((current) => ({
                  ...current,
                  tipo: event.target.value as ScheduleEntryKind,
                }))
              }
              value={form.tipo}
            >
          ...
          {isLesson ? (
            <Select
              ...
              name="salaId"
              onChange={(event) =>
                setForm((current) => ({ ...current, salaId: event.target.value }))
              }
              required
              value={form.salaId}
            >
          ...
          {!isEditing ? (
            <Select
              ...
              name="diaSemana"
              onChange={(event) =>
                setForm((current) => ({ ...current, diaSemana: event.target.value as Weekday }))
              }
          ...
          <Input
            ...
            name="horarioInicio"
            onChange={(event) =>
              setForm((current) => ({ ...current, horarioInicio: event.target.value }))
            }
            required
            type="time"
          ...
          <Input
            ...
            name="horarioFim"
            onChange={(event) =>
              setForm((current) => ({ ...current, horarioFim: event.target.value }))
            }
            required
            type="time"
            ...
            <div>
              ...
              <div className="mt-2 flex h-11 items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 text-sm font-semibold text-slate-600">
                <span
                  ...
                  style={{ backgroundColor: selectedSubject?.cor ?? '#e2e8f0' }}
                />
