...
export function Input({ className, error, label, id, ...props }: InputProps) {
  ...
  return (
    <label className="block" htmlFor={inputId}>
      ...
      <input
        className={cn(
          'mt-2 w-full rounded-2xl border bg-white px-4 py-3.5 text-sm font-medium text-brand-navy shadow-sm outline-none ring-1 ring-white/80 transition-all placeholder:text-slate-400 focus:-translate-y-0.5 focus:shadow-card focus:ring-4',
          error
