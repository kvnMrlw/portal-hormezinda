  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/45 px-4 backdrop-blur-sm">
      ...
      <section className="relative w-full max-w-lg overflow-hidden rounded-3xl bg-white p-6 text-center shadow-soft">
        <button className="absolute right-4 top-4 rounded-full p-2 text-slate-500 hover:bg-slate-100" onClick={() => setIsOpen(false)} type="button">
          <X className="h-5 w-5" />
        </button>
        ...
        <h2 className="mt-5 text-3xl font-semibold tracking-normal text-brand-navy">Feliz aniversário!</h2>
        <p className="mt-3 text-base font-medium leading-7 text-slate-600">🎉 Feliz aniversário! A equipe do Portal Hormezinda deseja um excelente dia!</p>
        <Button className="mt-6" onClick={() => setIsOpen(false)} type="button">Obrigado</Button>
      </section>
    </div>
  );
}
